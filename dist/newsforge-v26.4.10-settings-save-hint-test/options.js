...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function activateSettingsTab(tabId, options = {}) {
    const fallback = settingsTabs[0]?.dataset.tab || 'engine';
  ...
  try {
    activateSettingsTab(localStorage.getItem('newsforgeOptionsActiveTab') || 'engine', { restore: true });
  } catch (err) {
    activateSettingsTab('engine', { restore: true });
  }
