...
const EXPORT_STITCHED_RASTER_MAX_SIDE = 60000;
const EXPORT_STITCHED_RASTER_MAX_AREA = 180000000;
...
const ReaderRenderer = {
  ...
  _buildStitchedSvg({ tiles, background }) {
    ...
    const images = tiles.map(tile => {
      const currentY = y;
      y += tile.height;
      const href = this._escapeXmlAttr(tile.dataUrl);
      return `<image href="${href}" xlink:href="${href}" x="0" y="${currentY}" width="${tile.width}" height="${tile.height}" preserveAspectRatio="none"/>`;
    }).join('\n');
...
${images}
  ...
  _calculateSingleRasterExportScale(width, height) {
    const safeWidth = Math.max(1, width || 0);
    const safeHeight = Math.max(1, height || 0);
    const sideScale = Math.min(
      EXPORT_STITCHED_RASTER_MAX_SIDE / safeWidth,
      EXPORT_STITCHED_RASTER_MAX_SIDE / safeHeight
    );
    const areaScale = Math.sqrt(EXPORT_STITCHED_RASTER_MAX_AREA / (safeWidth * safeHeight));
    const scale = Math.min(EXPORT_CANVAS_SCALE, sideScale, areaScale);
    return Number.isFinite(scale) && scale > 0 ? scale : 1;
  },

  async _exportLongScreenshotAsRaster({ clone, theme, size, mime, ext, quality, timestamp, btn }) {
    const scale = this._calculateSingleRasterExportScale(size.width, size.height);
    this._setButtonLabel(btn, `Rendering ${ext.toUpperCase()} ${Math.round(scale * 100)}%`);
    ...
    if (scale < 0.95) {
      this.showToast(`Long article exported as downsampled ${ext.toUpperCase()} (${Math.round(scale * 100)}%)`);
    }
  },

  async _exportLongScreenshotAsSvg({ clone, theme, size, mime, quality, timestamp, btn, cuts }) {
    const tiles = [];
    for (let i = 0; i < cuts.length - 1; i++) {
      const y = cuts[i];
      const height = cuts[i + 1] - y;
      this._setButtonLabel(btn, `Stitching ${i + 1}/${cuts.length - 1}`);
      const canvas = await this._renderExportCanvas(clone, theme, { width: size.width, height, y });
      const flat = this._flattenCanvas(canvas, theme.bg);
      const dataUrl = await this._canvasToDataUrl(flat, mime, quality);
    ...
    this._downloadBlob(blob, `newsforge-${timestamp}.svg`);
    this.showToast(`Long article exported as one stitched SVG (${cuts.length - 1} slices)`);
  },
  ...
  async takeScreenshot() {
    ...
    try {
      ...
      if (size.height <= maxTileCssHeight) {
        ...
        try {
          await this._exportLongScreenshotAsRaster({
            clone,
            theme,
            size,
            mime,
          ...
          console.warn('[NewsForge] Long raster export failed, falling back to SVG:', error?.message || error);
