...
(function () {
  ...
  let floatReadinessPollTimer = null;
  let floatReadinessStopTimer = null;
  ...
  function syncAdapterToCurrentURL(force) {
    ...
    if (!url) return false;
    const changed = url !== currentPageURL || !currentAdapter;
  ...
  function evaluatePageState(force) {
    const urlChanged = syncAdapterToCurrentURL(force);
    if (currentAdapter && currentAdapter.isArticlePage()) {
      injectFloatIcon();
      startFloatReadinessWatcher(urlChanged);
      return true;
    }
  ...
  function injectFloatIcon() {
    ...
    floatIcon.addEventListener('click', (e) => {
      ...
      if (floatIcon.classList.contains('nf-float-loading')) {
        return;
      }
  ...
  function setFloatIconState(state) {
    if (!floatIcon) return;
    floatIcon.classList.remove('nf-float-pending', 'nf-float-loading', 'nf-float-ready', 'nf-float-error');
    floatIcon.dataset.state = state;

    if (state === 'loading') {
      floatIcon.classList.add('nf-float-loading');
      floatIcon.title = 'NewsForge - Preparing article...';
      return;
    }

    if (state === 'pending') {
      floatIcon.classList.add('nf-float-pending');
      floatIcon.title = 'NewsForge - Article is still loading. Click to retry.';
      return;
    }

    if (state === 'error') {
      floatIcon.classList.add('nf-float-error');
      floatIcon.title = 'NewsForge - Could not extract article. Try again after the page finishes loading.';
      return;
    }

    floatIcon.classList.add('nf-float-ready');
    floatIcon.title = 'NewsForge - Reader Mode';
  }

  function stopFloatReadinessWatcher() {
    if (floatReadinessPollTimer) {
      clearInterval(floatReadinessPollTimer);
      floatReadinessPollTimer = null;
    }
    if (floatReadinessStopTimer) {
      clearTimeout(floatReadinessStopTimer);
      floatReadinessStopTimer = null;
    }
  }

  function hasExtractableArticleContent() {
    try {
      syncAdapterToCurrentURL(true);
      if (!currentAdapter || !currentAdapter.isArticlePage()) return false;
      const paragraphs = currentAdapter.getParagraphs();
      return Array.isArray(paragraphs) && paragraphs.length > 0;
    } catch (e) {
      console.warn('[NewsForge] readiness check failed:', e);
      return false;
    }
  }

  function startFloatReadinessWatcher(force = false) {
    if (!floatIcon || ReaderRenderer.active || floatIcon.classList.contains('nf-float-loading')) return;
    if (!force && (floatReadinessPollTimer || floatIcon.dataset.state === 'ready')) return;
    stopFloatReadinessWatcher();
    setFloatIconState('pending');

    const check = () => {
      if (!floatIcon || ReaderRenderer.active || floatIcon.classList.contains('nf-float-loading')) {
        stopFloatReadinessWatcher();
        return;
      }
      if (hasExtractableArticleContent()) {
        setFloatIconState('ready');
        stopFloatReadinessWatcher();
      } else {
        setFloatIconState('pending');
      }
    ...
    setTimeout(check, 0);
    floatReadinessPollTimer = setInterval(check, 1500);
    floatReadinessStopTimer = setTimeout(() => {
      stopFloatReadinessWatcher();
      if (floatIcon && !ReaderRenderer.active && !floatIcon.classList.contains('nf-float-loading')) {
        setFloatIconState(hasExtractableArticleContent() ? 'ready' : 'pending');
      }
    }, 30000);
  }
  ...
  async function openReader(options) {
    ...
    try {
      ...
      setFloatIconState('error');
      showToast('Error extracting article: ' + (e.message || ''), 4000);
      setTimeout(startFloatReadinessWatcher, 1200);
      return;
    }
    ...
    if (paragraphs.length === 0) {
      ...
      setFloatIconState('error');
      showToast('Article is still loading. Please try again in a moment.');
      setTimeout(startFloatReadinessWatcher, 1200);
      return;
    }
    ...
    ReaderRenderer.onClose = () => {
      ...
      if (floatIcon) setFloatIconState('ready');
      startFloatReadinessWatcher();
    ...
    try {
      ...
      if (floatIcon) floatIcon.classList.remove('nf-hidden');
      if (floatIcon) setFloatIconState('error');
      setTimeout(startFloatReadinessWatcher, 1200);
      return;
    }
