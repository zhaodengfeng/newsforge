...
function buildNewsTranslationPrompt({ provider, model, langName, contentType, context, texts }) {
  ...
  if (isMTModel) {
    const systemPrompt = isHeadline
      ? `Translate the input news headline into ${langName}. Keep named entities consistent with the provided full-article terminology hints. Return only a JSON array of translated strings.`
      : `Translate the input news ${contentLabel} into ${langName}. Keep named entities consistent with the provided full-article terminology hints. Return only a JSON array of translated strings in the same order as the input array.`;

    const userPrompt = [
      contextBlock,
      'Use the context only to resolve ambiguity. Do not translate the instructions.',
      'For every person, organization, and place name, use one target-language form consistently across this article. If the provided segment has a partial name, resolve it against the full-name hints when possible. If you are not sure of the established Chinese characters for a romanized name, keep the romanized name instead of inventing characters.',
      'Input JSON array:',
...
5. Use any provided context and terminology hints only to resolve ambiguity.
6. If a romanized personal name has no clearly established target-language form, keep the romanized name instead of inventing Chinese characters.
...
6. Resolve surname-only or partial-name mentions against the full-name terminology hints when possible.
7. If a romanized personal name has no clearly established Chinese-script form, keep the romanized name instead of guessing characters.
8. Do not add honorifics such as Ms., Mr., 女士, or 先生 unless they are present in the source text.
9. Translate quotes faithfully without adding interpretation.
10. Keep section headings concise and news-style.
11. Use any provided context only to disambiguate meaning; do not introduce information not present in the segment itself.
12. Return translations in the same order as the input.
