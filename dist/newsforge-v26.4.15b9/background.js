...
async function translateProvider({ texts, targetLang, langName, provider, configOverride, promptContext, contentType }) {
  switch (provider) {
    ...
    default:
      try {
        return await openaiTranslate(texts, targetLang, langName, provider, configOverride, promptContext, contentType);
      } catch (err) {
        const isGemini = provider === 'gemini' || provider === 'openrouter';
        const isUnavailable = /503|529|unavailable|overloaded/i.test(String(err));
        if (isGemini && isUnavailable && texts.length > 0) {
          console.warn('[NewsForge] Gemini unavailable, falling back to Google Translate');
          return googleTranslate(texts, targetLang);
        }
        throw err;
      }
