...
class NewYorkerAdapter extends BaseAdapter {
  ...
  _findArticleEndMarker(container) {
    ...
    for (const el of allEls) {
      ...
      if (/^(read more|more:|more from the new yorker|more stories|recommended|the latest|popular|newsletter|sign up|subscribe|published in the print edition)/.test(lower)) {
        return el;
      }
    }
    return null;
  ...
  _isChromeOrPromo(el) {
    ...
    if (el.closest('[class*="newsletter" i], [class*="promo" i], [class*="ad-" i], [class*="-ad" i], [class*="advert" i], [class*="sponsor" i], [class*="recirc" i], [class*="Recirc" i], [class*="related" i], [class*="Related" i], [class*="tout" i], [class*="Tout" i], [class*="paywall" i], [class*="Paywall" i], [class*="social" i], [class*="Social" i], [class*="share" i], [class*="Share" i], [class*="caption" i], [class*="Caption" i]')) return true;

    const text = (el.innerText || '').replace(/\s+/g, ' ').trim();
    return /^(save this story|listen to this article|open navigation menu|search|buy or license this|view more|copy link|link copied)$/i.test(text);
  }

  _isBoilerplateText(text) {
    return /^(save this story|read more|more:|more from the new yorker|newsletter|sign up|subscribe|published in the print edition|the new yorker may earn|letters should be sent|buy or license this|view more|copy link|link copied|sections|more)$/i.test(text) ||
      /^©\s*\d{4}/i.test(text) ||
