...
class SCMPAdapter extends BaseAdapter {
  ...
  _isInlineSkipModule(el) {
    ...
    while (current && current !== document.body) {
      ...
      if (/oembed|methode-html-wrapper/.test(cls)) return true;
      if (/^(want to know more|read around this topic)/.test(text)) return true;

      if (current.tagName && /^(ARTICLE|MAIN)$/i.test(current.tagName)) break;
      current = current.parentElement;
    }
