...
const LLM_SHORT_ARTICLE_CHAR_LIMIT = 7500;
const LLM_SHORT_ARTICLE_ITEM_LIMIT = 40;
const LLM_MAX_CHUNK_ITEMS = 8;
const LLM_MAX_CHUNK_CHARS = 7500;
...
const ReaderRenderer = {
  ...
  buildTranslationChunks(texts, provider) {
    ...
    const totalChars = safeTexts.reduce((sum, text) => sum + text.length, 0);
    if (
      safeTexts.length > 0 &&
      safeTexts.length <= LLM_SHORT_ARTICLE_ITEM_LIMIT &&
      totalChars <= LLM_SHORT_ARTICLE_CHAR_LIMIT
    ) {
      return [{ start: 0, end: safeTexts.length }];
    }
    ...
    while (start < safeTexts.length) {
      ...
      while (end < safeTexts.length && end - start < LLM_MAX_CHUNK_ITEMS) {
        const nextLen = safeTexts[end].length;
        if (end > start && chars + nextLen > LLM_MAX_CHUNK_CHARS) break;
        chars += nextLen;
        end++;
      }
