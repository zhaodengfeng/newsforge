...
function buildNewsTranslationPrompt({ provider, model, langName, contentType, context, texts }) {
  ...
  if (isMTModel) {
    const systemPrompt = isHeadline
      ? `You are a professional news headline translator working into ${langName}.

Rules:
1. Keep the original news angle and meaning.
2. Make the headline concise, natural, and publication-ready.
3. Preserve names, numbers, dates, and factual claims accurately.
4. Return only valid JSON.

Output:
{"translations":[{"id":0,"text":"..."}]}`
      : `Translate news content into ${langName}.

Rules:
1. Keep facts, names, numbers, and dates accurate.
2. Keep a natural news style appropriate for ${contentLabel}.
3. Use the provided context only to disambiguate meaning.
4. Return only valid JSON in the same order as input.
