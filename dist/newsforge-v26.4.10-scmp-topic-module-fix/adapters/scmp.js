...
class SCMPAdapter extends BaseAdapter {
  ...
  _findArticleEndMarker(container) {
    ...
    for (let i = total - 1; i >= startIdx; i--) {
      ...
      if (/^(sign up|subscribe|newsletter|more from|related|recommended|popular|trending|you may also|readers also|keep reading|explore|more on this|more stories|want to know more|read around this topic|copyright|share your thoughts|join the conversation)/.test(lower)) {
        return allEls[i];
      }
    }
    return null;
  ...
  _isTerminalModule(el) {
    ...
    if (/piano-metering|paywall|swiper|related|topic|conversation|discover|recommend|voice-select|audio-player|oembed|methode-html-wrapper/i.test(cls)) {
      return true;
    }

    return /^(further reading|related topics|want to know more|read around this topic|before you go|discover more stories on|select voice|make scmp preferred on google)/.test(text);
  }
